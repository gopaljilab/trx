...
fn draw_settings_tab(frame: &mut Frame, app: &App, area: Rect, theme: &crate::config::Theme) {
    ...
    macro_rules! draw_setting {
        ($idx:expr, $label:expr, $value:expr, $is_toggle:expr) => {
            ...
            let indicator = if app.settings_index == $idx {
                "> "
            } else {
                "  "
            };
    ...
    draw_setting!(
        1,
        "Auto Update Check",
        if app.config.settings.auto_update_check {
            "true"
        } else {
            "false"
        },
    ...
    draw_setting!(
        2,
        "Auto Cleanup",
        if app.config.settings.auto_cleanup {
            "true"
        } else {
            "false"
        },
    ...
    draw_setting!(
        5,
        "Max Results",
        &app.config.settings.max_search_results.to_string(),
        false
    );
    settings_lines.push(Line::from(""));
    ...
    draw_setting!(
        current_idx + 2,
        "Spinner Type",
        &app.config.settings.spinner_type,
        false
    );
    current_idx += 3;
...
fn draw_help_overlay(frame: &mut Frame, app: &App, theme: &crate::config::Theme) {
    ...
    let help_text = vec![
        ...
        Line::from(vec![
            Span::styled(format!("{:<18}", &keys.quit), key_style),
            Span::raw("Quit"),
        ]),
        Line::from(""),
